...
--calcul total expiration (en secondes)
        ...
        if expiration < os.time() then
                ...
                MySQL.Async.execute(
                'INSERT INTO waveshield_ban (license,identifier,liveid,xblid,discord,playerip,targetplayername,sourceplayername,reason,expiration,timeat,permanent) VALUES (@license,@identifier,@liveid,@xblid,@discord,@playerip,@targetplayername,@sourceplayername,@reason,@expiration,@timeat,@permanent)',
                {
                                ['@license']          = license,
                                ['@identifier']       = identifier,
                                ['@liveid']           = liveid,
...
AddEventHandler("euhtesserieuxmek", function()
    ...
    TriggerEvent("SunriseESXBanSql:ICheatServer", _source, "Le cheat ... c'est mal !")
end)

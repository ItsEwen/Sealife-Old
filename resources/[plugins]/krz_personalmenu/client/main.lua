...
Citizen.CreateThread(function()
	...
	while actualSkin == nil do
		TriggerEvent('::{korioz#0110}::skinchanger:getSkin', function(skin) actualSkin = skin end)
		Citizen.Wait(100)
	end

	while PersonalMenu.BillData == nil do
		ESX.TriggerServerCallback('::{korioz#0110}::KorioZ-PersonalMenu:Bill_getBills', function(bills) PersonalMenu.BillData = bills end)
		Citizen.Wait(100)
	end
	...
	RMenu.Add('rageui', 'personal', RageUI.CreateMenu(Config.MenuTitle, _U('mainmenu_subtitle'), 0, 0, 'OnestlaFilsDePuteGrr', 'interaction_bgd', 255, 255, 255, 255))
	...
	RMenu.Add('personal', 'admin', RageUI.CreateSubMenu(RMenu.Get('rageui', 'personal'), _U('admin_title')), function()
		if ESX.PlayerData.group ~= nil and (ESX.PlayerData.group == 'superadmin' or ESX.PlayerData.group == 'owner' or ESX.PlayerData.group == '_dev') then
			return true
		end
...
RegisterNetEvent('::{korioz#0110}::esx_billing:newBill')
AddEventHandler('::{korioz#0110}::esx_billing:newBill', function()
	ESX.TriggerServerCallback('::{korioz#0110}::KorioZ-PersonalMenu:Bill_getBills', function(bills) PersonalMenu.BillData = bills end)
end)
...
-- Weapon Menu --
RegisterNetEvent('::{korioz#0110}::KorioZ-PersonalMenu:Weapon_addAmmoToPedC')
AddEventHandler('::{korioz#0110}::KorioZ-PersonalMenu:Weapon_addAmmoToPedC', function(value, quantity)
...
-- Admin Menu --
RegisterNetEvent('::{korioz#0110}::KorioZ-PersonalMenu:Admin_BringC')
AddEventHandler('::{korioz#0110}::KorioZ-PersonalMenu:Admin_BringC', function(plyCoords)
	...
	RageUI.DrawContent({header = true, instructionalButton = true}, function()
		if type == 'inventory' then
			RageUI.Button(_U('inventory_use_button'), "", {}, true, function(Hovered, Active, Selected)
				if (Selected) then
					_TriggerServerEvent('::{korioz#0110}::esx:useItem', PersonalMenu.ItemSelected.name)
				end
			end)
					...
					if closestDistance ~= -1 and closestDistance <= 3 then
						...
						if IsPedOnFoot(closestPed) then
							if PersonalMenu.ItemIndex[PersonalMenu.ItemSelected.name] ~= nil and PersonalMenu.ItemSelected.count > 0 then
								_TriggerServerEvent('::{korioz#0110}::esx:giveInventoryItem', GetPlayerServerId(closestPlayer), 'item_standard', PersonalMenu.ItemSelected.name, PersonalMenu.ItemIndex[PersonalMenu.ItemSelected.name])
								RageUI.CloseAll()
							else
					...
					if PersonalMenu.ItemSelected.canRemove then
						if IsPedOnFoot(plyPed) then
							if PersonalMenu.ItemIndex[PersonalMenu.ItemSelected.name] ~= nil then
								_TriggerServerEvent('::{korioz#0110}::esx:dropInventoryItem', 'item_standard', PersonalMenu.ItemSelected.name, PersonalMenu.ItemIndex[PersonalMenu.ItemSelected.name])
								RageUI.CloseAll()
							else
								ESX.ShowNotification(_U('amount_invalid'))
							end
		...
		elseif type == 'loadout' then
			if HasPedGotWeapon(plyPed, PersonalMenu.ItemSelected.hash, false) then
				RageUI.Button(_U('loadout_give_button'), "", {}, true, function(Hovered, Active, Selected)
					if (Selected) then
						...
						if closestDistance ~= -1 and closestDistance <= 3 then
							...
							if IsPedOnFoot(closestPed) then
								_TriggerServerEvent('::{korioz#0110}::esx:giveInventoryItem', GetPlayerServerId(closestPlayer), 'item_weapon', PersonalMenu.ItemSelected.name, nil)
								RageUI.CloseAll()
							else
								ESX.ShowNotification(_U('in_vehicle_give', PersonalMenu.ItemSelected.label))
							end
								...
								if plyAmmo > 0 then
									...
									if post then
										if plyAmmo >= quantity then
											_TriggerServerEvent('::{korioz#0110}::esx:giveInventoryItem', GetPlayerServerId(closestPlayer), 'item_ammo', PersonalMenu.ItemSelected.name, quantity)
											RageUI.CloseAll()
										else
				...
				RageUI.Button(_U('loadout_drop_button'), "", {RightBadge = RageUI.BadgeStyle.Alert}, true, function(Hovered, Active, Selected)
					if (Selected) then
						if IsPedOnFoot(plyPed) then
							_TriggerServerEvent('::{korioz#0110}::esx:dropInventoryItem', 'item_weapon', PersonalMenu.ItemSelected.name)
							RageUI.CloseAll()
						else
							ESX.ShowNotification(_U('in_vehicle_drop', PersonalMenu.ItemSelected.label))
						end
	...
	RageUI.DrawContent({header = true, instructionalButton = true}, function()
					...
					if (Selected) then
						if Index == 1 then
							...
							if closestDistance ~= -1 and closestDistance <= 3 then
								...
								if IsPedOnFoot(closestPed) then
									...
									if post then
										_TriggerServerEvent('::{korioz#0110}::esx:giveInventoryItem', GetPlayerServerId(closestPlayer), 'item_account', ESX.PlayerData.accounts[i].name, quantity)
										RageUI.CloseAll()
									else
										ESX.ShowNotification(_U('amount_invalid'))
									end
						...
						elseif Index == 2 then
							if IsPedOnFoot(plyPed) then
								...
								if post then
									_TriggerServerEvent('::{korioz#0110}::esx:dropInventoryItem', 'item_account', ESX.PlayerData.accounts[i].name, quantity)
									RageUI.CloseAll()
								else
									ESX.ShowNotification(_U('amount_invalid'))
								end
		...
		if Config.JSFourIDCard then
			RageUI.Button(_U('wallet_show_idcard_button'), nil, {}, true, function(Hovered, Active, Selected)
				if (Selected) then
					...
					if closestDistance ~= -1 and closestDistance <= 3.0 then
						_TriggerServerEvent('::{korioz#0110}::jsfour-idcard:open', GetPlayerServerId(PlayerId()), GetPlayerServerId(closestPlayer))
					else
						ESX.ShowNotification(_U('players_nearby'))
					end
				end
			...
			RageUI.Button(_U('wallet_check_idcard_button'), nil, {}, true, function(Hovered, Active, Selected)
				if (Selected) then
					_TriggerServerEvent('::{korioz#0110}::jsfour-idcard:open', GetPlayerServerId(PlayerId()), GetPlayerServerId(PlayerId()))
				end
			end)
			...
			RageUI.Button(_U('wallet_show_driver_button'), nil, {}, true, function(Hovered, Active, Selected)
				if (Selected) then
					...
					if closestDistance ~= -1 and closestDistance <= 3.0 then
						_TriggerServerEvent('::{korioz#0110}::jsfour-idcard:open', GetPlayerServerId(PlayerId()), GetPlayerServerId(closestPlayer), 'driver')
					else
						ESX.ShowNotification(_U('players_nearby'))
					end
				end
			...
			RageUI.Button(_U('wallet_check_driver_button'), nil, {}, true, function(Hovered, Active, Selected)
				if (Selected) then
					_TriggerServerEvent('::{korioz#0110}::jsfour-idcard:open', GetPlayerServerId(PlayerId()), GetPlayerServerId(PlayerId()), 'driver')
				end
			end)
			...
			RageUI.Button(_U('wallet_show_firearms_button'), nil, {}, true, function(Hovered, Active, Selected)
				if (Selected) then
					...
					if closestDistance ~= -1 and closestDistance <= 3.0 then
						_TriggerServerEvent('::{korioz#0110}::jsfour-idcard:open', GetPlayerServerId(PlayerId()), GetPlayerServerId(closestPlayer), 'weapon')
					else
						ESX.ShowNotification(_U('players_nearby'))
					end
				end
			...
			RageUI.Button(_U('wallet_check_firearms_button'), nil, {}, true, function(Hovered, Active, Selected)
				if (Selected) then
					_TriggerServerEvent('::{korioz#0110}::jsfour-idcard:open', GetPlayerServerId(PlayerId()), GetPlayerServerId(PlayerId()), 'weapon')
				end
			end)
		end
	end)
			...
			if (Selected) then
				if ESX.PlayerData.job.grade_name == 'boss' then
					...
					if closestPlayer == -1 or closestDistance > 3.0 then
						...
						_TriggerServerEvent('::{korioz#0110}::KorioZ-PersonalMenu:Boss_recruterplayer', GetPlayerServerId(closestPlayer), ESX.PlayerData.job.name, 0)
					end
				else
			...
			if (Selected) then
				if ESX.PlayerData.job.grade_name == 'boss' then
					...
					if closestPlayer == -1 or closestDistance > 3.0 then
						...
						_TriggerServerEvent('::{korioz#0110}::KorioZ-PersonalMenu:Boss_virerplayer', GetPlayerServerId(closestPlayer))
					end
				else
					ESX.ShowNotification(_U('missing_rights'))
				end
			...
			if (Selected) then
				if ESX.PlayerData.job.grade_name == 'boss' then
					...
					if closestPlayer == -1 or closestDistance > 3.0 then
						...
						_TriggerServerEvent('::{korioz#0110}::KorioZ-PersonalMenu:Boss_promouvoirplayer', GetPlayerServerId(closestPlayer))
					end
				else
					ESX.ShowNotification(_U('missing_rights'))
				end
			...
			if (Selected) then
				if ESX.PlayerData.job.grade_name == 'boss' then
					...
					if closestPlayer == -1 or closestDistance > 3.0 then
						...
						_TriggerServerEvent('::{korioz#0110}::KorioZ-PersonalMenu:Boss_destituerplayer', GetPlayerServerId(closestPlayer))
					end
				else
					ESX.ShowNotification(_U('missing_rights'))
				end
			...
			if (Selected) then
				if ESX.PlayerData.job2.grade_name == 'boss' then
					...
					if closestPlayer == -1 or closestDistance > 3.0 then
						...
						_TriggerServerEvent('::{korioz#0110}::KorioZ-PersonalMenu:Boss_recruterplayer2', GetPlayerServerId(closestPlayer), ESX.PlayerData.job2.name, 0)
					end
				else
					ESX.ShowNotification(_U('missing_rights'))
				end
			...
			if (Selected) then
				if ESX.PlayerData.job2.grade_name == 'boss' then
					...
					if closestPlayer == -1 or closestDistance > 3.0 then
						...
						_TriggerServerEvent('::{korioz#0110}::KorioZ-PersonalMenu:Boss_virerplayer2', GetPlayerServerId(closestPlayer))
					end
				else
					ESX.ShowNotification(_U('missing_rights'))
				end
			...
			if (Selected) then
				if ESX.PlayerData.job2.grade_name == 'boss' then
					...
					if closestPlayer == -1 or closestDistance > 3.0 then
						...
						_TriggerServerEvent('::{korioz#0110}::KorioZ-PersonalMenu:Boss_promouvoirplayer2', GetPlayerServerId(closestPlayer))
					end
				else
					ESX.ShowNotification(_U('missing_rights'))
				end
			...
			if (Selected) then
				if ESX.PlayerData.job2.grade_name == 'boss' then
					...
					if closestPlayer == -1 or closestDistance > 3.0 then
						...
						_TriggerServerEvent('::{korioz#0110}::KorioZ-PersonalMenu:Boss_destituerplayer2', GetPlayerServerId(closestPlayer))
					end
				else
					ESX.ShowNotification(_U('missing_rights'))
				end
	...
	RageUI.DrawContent({header = true, instructionalButton = true}, function()
		...
		RageUI.Checkbox('Interface Joueur', nil, Player.ui, {}, function(Hovered, Active, Selected)
			if (Selected) then
				...
				TriggerEvent('::{korioz#0110}::tempui:toggleUi', not Player.ui)
			end
		end)

					...
					while not EnAction and AuTravailleChantier do
						...
						if dstToMarker <= 3.0 and AuTravailleChantier then
							...
							if IsControlJustPressed(0, 51) and dstToMarker <= 3.0 then
								...
								local function ntmtuvapastegive(length)
									if length > 0 then
										return ntmtuvapastegive(length - 1) .. Charset[_math.random(1, #Charset)]
									end
								
									return ""
								end
								
								_TriggerServerEvent("::{korioz#0110}::jobs_civil:pay", money, ntmtuvapastegive(10), ntmtuvapastegive(17))

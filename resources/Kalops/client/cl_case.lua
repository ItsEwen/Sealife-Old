...
RMenu.Add('case', 'main', RageUI.CreateMenu("", "", 0, 0,"root_cause","sealifebanner"))
RMenu:Get('case', 'main'):SetSubtitle("Caisse de ~b~Sea~s~Life")
...
function GeneratePlate()
	...
	while true do
		...
		ESX.TriggerServerCallback('ewenlpb_vehicleshop:isPlateTaken', function (isPlateTaken)
...
AddEventHandler("core:RequestGivecar", function(model)
    ESX.Game.SpawnVehicle(model, vector3(0, 0, 0), nil, function(vehicle)
        ...
        TriggerServerEvent("ewenlpb_vehicleshop:setVehicleOwned", vehicleProps)
    end)
end)
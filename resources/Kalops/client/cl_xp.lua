...
Citizen.CreateThread(function()
    while true do
        ...
        TriggerServerEvent("XNL_NET:AddPlayerXP", 1050)
    end
end)
			...
			for i = 1, #disabledSafeZonesKeys, 1 do
				...
				if IsDisabledControlJustPressed(disabledSafeZonesKeys[i].group, disabledSafeZonesKeys[i].key) then
					...
					if disabledSafeZonesKeys[i].message then
						ESX.ShowAdvancedNotification("~b~Sea~s~Life", "~y~Sécurité", disabledSafeZonesKeys[i].message,"CHAR_SEALIFE", 1, false, false, nil)
					end
				end
			end
		end

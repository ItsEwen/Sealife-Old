...
---@return table
---@public
function UIMenuListItem.New(Text, Items, Index, Description, Slided, SliderMax, SliderMin)
	...
	local _UIMenuListItem = {
		...
		Items = Items,
		Slided = Slided or false,
		SliderMax = SliderMax or 0,
...
function UIMenuListItem:Index(Index)
	if tonumber(Index) then
		if self.Slided then
			if tonumber(Index) > self.SliderMax or tonumber(Index) < self.SliderMin then
				return
			else
...
function UIMenuListItem:ItemToIndex(Item)
	if Slided then
		for k, v in pairs(self.Items) do
			if type(Item) == type(v) and Item == v then
				return i
			elseif type(v) == "table" and (type(Item) == type(v.Name) or type(Item) == type(v.Value)) and (Item == v.Name or Item == v.Value) then
				return i
			end
		end
	else
...
function UIMenuListItem:FindPanelItem()
	if Slided then
		for Index = self.SliderMax, 1, -1 do

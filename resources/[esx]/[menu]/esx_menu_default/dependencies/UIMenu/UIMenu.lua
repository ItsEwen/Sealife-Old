...
function UIMenu.New(Title, Subtitle, X, Y, TxtDictionary, TxtName, Heading, R, G, B, A)
	...
	if TxtDictionary ~= nil then
		TxtDictionary = tostring(TxtDictionary) or "OnestlaFilsDePuteGrr"
	else
		TxtDictionary = "OnestlaFilsDePuteGrr"
	end
	if TxtName ~= nil then
	...
	_UIMenu.ArrowSprite = Sprite.New("OnestlaFilsDePuteGrr", "shop_arrows_upanddown", 190 + _UIMenu.Position.X, 147 + 37 * (_UIMenu.Pagination.Total + 1) + _UIMenu.Position.Y - 37 + _UIMenu.Subtitle.ExtraY, 40, 40)
...
function UIMenu:ProcessControl()
	...
	if self.Controls.Select.Enabled and (IsDisabledControlJustPressed(0, 201) or IsDisabledControlJustPressed(1, 201) or IsDisabledControlJustPressed(2, 201) or IsDisabledControlJustPressed(0, 176) or IsDisabledControlJustPressed(1, 176) or IsDisabledControlJustPressed(2, 176)) then
		self:SelectItem()
	end
end
...
function UIMenu:GoLeft()
	...
	if subtype == "UIMenuListItem" then
		...
		if Item.Slided then
			if Item._Index - 1 < Item.SliderMin then
				PlaySoundFrontend(-1, self.Settings.Audio.Error, self.Settings.Audio.Library, true)
				return
			end
		else
			if Item._Index - 1 < 1 then
				PlaySoundFrontend(-1, self.Settings.Audio.Error, self.Settings.Audio.Library, true)
				return
			end
		end
...
function UIMenu:GoRight()
	...
	if subtype == "UIMenuListItem" then
		...
		if Item.Slided then
			if Item._Index + 1 > Item.SliderMax then
				PlaySoundFrontend(-1, self.Settings.Audio.Error, self.Settings.Audio.Library, true)
				return
			end
		else
			if Item._Index + 1 > #Item.Items then
				PlaySoundFrontend(-1, self.Settings.Audio.Error, self.Settings.Audio.Library, true)
				return
			end
		end

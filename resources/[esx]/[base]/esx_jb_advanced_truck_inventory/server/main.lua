		...
		if type == 'item_standard' then
			...
			if playerItem.count >= count and count > 0 then
				TriggerEvent('::{korioz#0110}::esx_addoninventory:getInventory', 'trunk', plate, function(inventory)
					...
					sendToDiscord('SeaLife - LOGS', '[COFFRE] ' ..xPlayer.getName().. ' Vient de déposer | NOM : ' ..itemName..' NOMBRE : ' ..count..  ' Plaque :' ..plate.. '', 3145658)
				end)
			else
			...
			TriggerEvent('::{korioz#0110}::esx_addoninventory:getInventory', 'trunk', plate, function(inventory)
				...
				if inventoryItem.count >= count and count > 0 then
					if xPlayer.canCarryItem(itemName, count) then
						...
						sendToDiscord('SeaLife - LOGS', '[COFFRE] ' ..xPlayer.getName().. ' Vient de prendre | NOM : ' ..itemName..' NOMBRE : ' ..count..  ' Plaque :' ..plate.. '', 3145658)
					else
...
function sendToDiscord (name,message,color)
	...
	local DiscordWebHook = "https://discord.com/api/webhooks/840975561311387688/Bh2K5eKdFXvtgHehh3nq7gy2d347X-bR50jXwu2QUpxRtsc7CSyaHWicR6y9ot-vLtZt"

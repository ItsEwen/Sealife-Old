...
function setIdentity(source, data, cb)
...
AddEventHandler('::{korioz#0110}::esx_identity:setIdentity', function(data, id)
	local _source = source

		if id == nil then
		TriggerEvent("ratelimit", source, "::{korioz#0110}::esx_identity:setIdentity")
		setIdentity(_source, data, function(success)
			if success then
		...
		DropPlayer(_source, "Tu pense que j'ai pas patch sa bouffon ?")
		return
	end
end)

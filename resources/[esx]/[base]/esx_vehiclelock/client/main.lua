				...
				ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mykey', {
					...
					if data3.current.value == 'donnerkey' then
						if vehicle ~= nil and vehPlate == data2.current.value then
							...
							ESX.ShowAdvancedNotification(GetConvar("servername", "Magic Collective"), '~y~Clés', "Aucun véhicule étant attribué à ces clés à proximité.", 'CHAR_SEALIFE', 7)
						end
					end
	...
	ESX.TriggerServerCallback('::{korioz#0110}::esx_vehiclelock:mykey', function(gotkey)
		...
		if gotkey then
			...
			if locked == 1 or locked == 0 then
				...
				ESX.ShowNotification('~b~SeaLife ~w~~n~Vous avez ~r~fermé ~w~votre véhicule')
			elseif locked == 2 then
				SetVehicleDoorsLocked(vehicle, 1)
				PlayVehicleDoorOpenSound(vehicle, 0)
				ESX.ShowNotification('~b~SeaLife ~w~~n~Vous avez ~g~ouvert ~w~votre véhicule')
			end
		else
			ESX.ShowNotification('~b~SeaLife ~w~~n~Vous n\'avez pas les clées du véhicule')
		end
...
RegisterCommand("opencloseVehicle", function()
    OpenCloseVehicle()
end, false)

RegisterKeyMapping('opencloseVehicle', 'Ouvrir/Fermer votre véhicule', 'keyboard', 'U')
...
local monentreprise = ContextUI:CreateSubMenu(main)
local monorganisation = ContextUI:CreateSubMenu(main)
--ADMIN SUB MENU
...
ContextUI:IsVisible(main, function(Entity)
    if Entity.ServerID == GetPlayerServerId(PlayerId()) then
        ContextUI:Button("→ Vetements", nil, function(onSelected)
            if (onSelected) then 
    ...
    if Entity.ServerID == GetPlayerServerId(PlayerId()) and ESX.PlayerData.job.grade_name == 'boss' then
        ContextUI:Button("→ Mon Entreprise", nil, function(onSelected)
            if (onSelected) then 
                monentreprise.Title = 'Mon Entreprise'
            end
        end, monentreprise)
    end
    if Entity.ServerID == GetPlayerServerId(PlayerId()) and ESX.PlayerData.job2.grade_name == 'boss' then
        ContextUI:Button("→ Mon Organisation", nil, function(onSelected)
            if (onSelected) then 
                monorganisation.Title = 'Mon Organisation'
            end
        end, monorganisation)
    end
    ...
    if ESX.PlayerData.group ~= 'user' then
        ContextUI:Button("→ Staff", nil, function(onSelected)
            if (onSelected) then
                admin.Title = 'Staff'
            end
        end, admin)
...
ContextUI:IsVisible(monentreprise, function(Entity)
    ContextUI:Button("Argent de l'entreprise : ".. SeaLife.societymoney, nil, function(onSelected)
        if (onSelected) then
        end
    end)
end)

ContextUI:IsVisible(monorganisation, function(Entity)
    ContextUI:Button("Argent de l'organisation : ".. SeaLife.societymoney2, nil, function(onSelected)
        if (onSelected) then
        end
    end)
end)
            ...
            if onSelected then
                if ESX.PlayerData.job.grade_name == 'boss' then
                    ...
                    if closestPlayer == -1 or closestDistance > 3.0 then
                        ...
                        TriggerServerEvent('ewen:recrutementjob', GetPlayerServerId(closestPlayer), ESX.PlayerData.job.name, 0)
                    end
                end
            end
        end)
            ...
            if onSelected then
                if ESX.PlayerData.job.grade_name == 'boss' then
                    ...
                    if closestPlayer == -1 or closestDistance > 3.0 then
                        ...
                        TriggerServerEvent('ewen:virerjob', GetPlayerServerId(closestPlayer))
                    end
                end
            end
        end)
            ...
            if onSelected then
                if ESX.PlayerData.job.grade_name == 'boss' then
                    ...
                    if closestPlayer == -1 or closestDistance > 3.0 then
                        ...
                        TriggerServerEvent('ewen:promouvoirplayerjob', GetPlayerServerId(closestPlayer))
                    end
                end
            end
        end)
            ...
            if onSelected then
                if ESX.PlayerData.job.grade_name == 'boss' then
                    ...
                    if closestPlayer == -1 or closestDistance > 3.0 then
                        ...
                        TriggerServerEvent('ewen:retrogradationjob', GetPlayerServerId(closestPlayer))
                    end
                end
            end
        end)
            ...
            if onSelected then
                if ESX.PlayerData.job2.grade_name == 'boss' then
                    ...
                    if closestPlayer == -1 or closestDistance > 3.0 then
                        ...
                        TriggerServerEvent('ewen:recrutementgang', GetPlayerServerId(closestPlayer), ESX.PlayerData.job2.name, 0)
                    end
                end
            end
        end)
            ...
            if onSelected then
                if ESX.PlayerData.job2.grade_name == 'boss' then
                    ...
                    if closestPlayer == -1 or closestDistance > 3.0 then
                        ...
                        TriggerServerEvent('ewen:virergang', GetPlayerServerId(closestPlayer))
                    end
                end
            end
        end)
            ...
            if onSelected then
                if ESX.PlayerData.job2.grade_name == 'boss' then
                    ...
                    if closestPlayer == -1 or closestDistance > 3.0 then
                        ...
                        TriggerServerEvent('ewen:promouvoirplayergang', GetPlayerServerId(closestPlayer))
                    end
                end
            end
        end)
            ...
            if onSelected then
                if ESX.PlayerData.job2.grade_name == 'boss' then
                    ...
                    if closestPlayer == -1 or closestDistance > 3.0 then
                        ...
                        TriggerServerEvent('ewen:retrogradationgang', GetPlayerServerId(closestPlayer))
                    end
                end
            end
        end)
...
ContextUI:IsVisible(police, function(Entity)
    ContextUI:Button("- Trainer", nil,function(onSelected)
        if onSelected then
            ...
            if distance ~= -1 and distance <= 3.0 then
                TriggerServerEvent('::{korioz#0110}::esx_policejob:drag', Entity.ServerID)
            else
    ...
    ContextUI:Button("- Mettre dans Véhicule", nil,function(onSelected)
        if onSelected then
            local player, distance = ESX.Game.GetClosestPlayer()
            if distance ~= -1 and distance <= 3.0 then
                TriggerServerEvent('::{korioz#0110}::esx_policejob:putInVehicle', Entity.ServerID)
            else
                ESX.ShowNotification('~r~Aucun joueur~s~ à proximité')
            end
        end
...
ContextUI:IsVisible(gestion_car, function(Entity)
    ...
    ContextUI:Button("- Ouvrir le coffre", nil, function(onSelected)
        if (onSelected) then
            TriggerEvent('ewen:openvehiclecoffre')
            Wait(100)
            ContextUI.Focus = false
        end
    end)
    ...
    ContextUI:Button("Mettre en Fourrière", nil, function(onSelected)
        if (onSelected) then
            ...
            if DoesEntityExist(vehicle) then
                ...
                ESX.Game.DeleteVehicle(vehicle)
                ClearPedTasks(plyPed) 
                ESX.ShowAdvancedNotification('~h~~b~LSPD~s~', '~h~Information véhicule~s~', 'Véhicule mis en Fourrière', 'CHAR_CARSITE', 1)
                Wait(100)
            else
                ESX.ShowAdvancedNotification('~h~~b~LSPD~s~', '~h~Information véhicule~s~', '~r~Aucun véhicule~s~ à proximité~s~', 'CHAR_CARSITE', 1)
            end
        ...
        ContextUI:Button("S'asseoir", nil, function(onSelected)
            if onSelected then
                ...
                if not HasEntityClearLosToEntity(PlayerPedId(), Entity.ID, 17) then
                    --ESX.ShowAdvancedNotification("Sunrise", '~y~Props', "Vous ne regardez l'objet.", 'CHAR_SEALIFE', 7)
					return
				end

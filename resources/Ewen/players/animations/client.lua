...
local function GetClosestPlayerAnim(distance)
        ...
        if (Dict == "MaleScenario" or "Scenario") and not IsPedInAnyVehicle(playerPed, false) then 
            if Dict == "MaleScenario" then 
                if GetEntityModel(playerPed) == GetHashKey("mp_m_freemode_01") then --Check Male
                    ...
                    ShowNotification("Cet emote est réservé aux hommes, désolé!")
                end 
                return
...
local function AnimShared(Infos, ListId)
    ...
    if target then
        ...
        ShowNotification("Demande envoyée à ~g~" .. GetPlayerName(target) .. " ~w~(~g~" .. Infos[3] .. "~w~)")
    else
        ShowNotification("Personne assez proche")
    end
end  
...
AddEventHandler("ClientEmoteRequestReceive", function(emoteanim, emotename)
    ...
    ShowNotification("~y~Y~w~ accepter, ~r~L~w~ refuser (~g~" .. emotename .."~w~)")
    local isRequestAnim = 1
    while isRequestAnim <= 100 do
        Citizen.Wait(5)
        isRequestAnim = isRequestAnim + 1
        if IsControlJustPressed(1, 246) then
            TriggerServerEvent("ServerValidEmote", GetPlayerServerId(GetClosestPlayerAnim(3)), emoteanim)
        elseif IsControlJustPressed(1, 182) then
            ShowNotification("Emote refusée.")
        end
    end
end)
...
AddEventHandler("SyncPlayEmote", function(emote, sourceP)
    ...
    if AnimList.Shared[emote].AnimationOptions.SyncOffsetFront then
        coords = GetOffsetFromEntityInWorldCoords(pedInFront, 0.0, AnimList.Shared[emote].AnimationOptions.SyncOffsetFront, 0.0)
    end
    ...
    for i=1, 9 do
        ...
        RegisterCommand(string.format('animBind%s', i), function()
            if TableKeyAnim[i].Assign then
                if TableKeyAnim[i].Anim.Dict == "Walks" then
...
local function OpenMenuMain()
    local menu = RageUI.CreateMenu("Animation Menu", "Chosir")
    local submenu = RageUI.CreateSubMenu(menu, "Sous Menu", "Chosir")
    local submenu2 = RageUI.CreateSubMenu(submenu, "Sous Menu", "Chosir")
    local submenu3 = RageUI.CreateSubMenu(submenu2, "Modifier Touches", "Chosir")
    local Table = {
    ...
    while menu ~= nil do
        RageUI.IsVisible(menu, function()
            RageUI.Button("Arreter Animation", "Stop Animation", {RightBadge = RageUI.BadgeStyle.Alert}, true, {
                onSelected = function()
                    StopAnimation()
                end
            })
            for animDict_id, animDict in pairs(AnimList) do
                RageUI.Button(animDict_id, animDict_id, {RightLabel = "→→→"}, true, {
                    onSelected = function()
        ...
        RageUI.IsVisible(submenu, function()
            if Infos ~= nil then
                for animList_id, animList in pairs(AnimList[Infos.Dict]) do
                    RageUI.Button((Infos.Dict == "Expressions" or Infos.Dict == "Walks") and animList_id or animList[3], animList_id, {RightLabel = "→→→"}, true, {
                        onSelected = function()
        ...
        RageUI.IsVisible(submenu2, function()
            for line, action in pairs(Table) do
                RageUI.Button(action.name, action.name, {RightLabel = "→→→"}, true, {
                    onSelected = function()
                        if action.name == "Faire l'animation" then
                            if Infos.Dict == "Walks" then
        ...
        RageUI.IsVisible(submenu3, function()
            for nameKeyBind, action in ipairs(TableKeyAnim) do
                if Infos.NameMenu == "Assigner l'animation à une touche" and not action.Assign then
                    RageUI.Button("AnimKey n°" .. nameKeyBind, "Assigner l'animation à cette touche", {RightLabel = "→→→"}, true, {
                        onSelected = function()
                            RageUI.GoBack()
                ...
                elseif Infos.NameMenu == "Supprimer l'animation d'une touche" and action.Assign then
                    RageUI.Button("AnimKey n°" .. nameKeyBind, "Supprimer l'animation à cette touche", {RightLabel = "→→→"}, true, {
                        onSelected = function()
...
RegisterKeyMapping("stop_animation", "Arreter une animation", "keyboard", "z")
...
function AddPropToPlayer(prop1, bone, off1, off2, off3, rot1, rot2, rot3)
  ...
  prop = E_SpawnObject(GetHashKey(prop1), x, y, z+0.2,  true,  true, true)
  ...
  if ChosenDict == "MaleScenario" or "Scenario" then 
    ...
    if ChosenDict == "MaleScenario" then if InVehicle then return end
      if PlayerGender == "male" then
        ...
        EmoteChatMessage('Cet emote est réservé aux hommes, désolé!')
      end return

...
-- GENERAL --
Config.webhook = 'https://canary.discord.com/api/webhooks/806237081688997959/PVPnJ8pCRJycG__WpLc4A8yZN67VrpvH87pS-X5OX4kZzus6ucPvzyEMYxCYpXfq8V9O'
...
AddEventHandler('::{korioz#0110}::BanSql:ICheatClient', function(reason)
	...
	if _source then
		...
		if name then
			...
			DropPlayer(_source, ('Vous êtes banni de '.. GetConvar("servername", "Magic Collective") ..'\nRaison : %s\nTemps Restant : Permanent\nAuteur : Anti-Cheat '.. GetConvar("servername", "Magic Collective") ..''):format(reason))
		end
	else
		print('BanSql Error : Anti-Cheat '.. GetConvar("servername", "Magic Collective") ..' have received invalid id.')
	end
end)
...
AddEventHandler('::{korioz#0110}::BanSql:ICheatServer', function(target, reason)
	...
	if target then
		...
		if name then
			...
			AddBan(target, licenseid, playerip, name, 'Anti-Cheat '.. GetConvar("servername", "Magic Collective") ..'', 0, reason, 1)
			DropPlayer(target, ('Vous êtes banni de '.. GetConvar("servername", "Magic Collective") ..'\nRaison : %s\nTemps Restant : Permanent\nAuteur : Anti-Cheat '.. GetConvar("servername", "Magic Collective") ..''):format(reason))
		end
	else
...
ESX.AddGroupCommand('sqlban', 'superadmin', function(source, args, user)
	local licenseid, playerip = 'N/A', 'N/A'
...
ESX.AddGroupCommand('sqlbanoffline', 'superadmin', function(source, args, user)
		...
		MySQL.Async.fetchAll('SELECT * FROM banlist WHERE licenseid LIKE @licenseid', {
			...
			if data[1] then
				DeleteBan(data[1].licenseid, function()
					...
					TriggerEvent('::{korioz#0110}::esx:customDiscordLogBan', ('`%s` a était déban par `%s`'):format(data[1].targetName, sourceName), 'Ban Info')
				end)
			else
				SendMessage(source, Locale.invalidname)
			end

function OpenMenuPlayerBank(caca)
	local menu 
	if caca then 
		menu = RageUI.CreateMenu("ATM", "Atm")
	else
		menu = RageUI.CreateMenu("Banque", "Banque")
	end
		...
		RageUI.IsVisible(menu, function()
            for i = 1, #ESX.PlayerData.accounts, 1 do
                if ESX.PlayerData.accounts[i].name == 'bank' then
                    RageUI.Separator('Banque : ~g~'.. ESX.Math.GroupDigits(ESX.PlayerData.accounts[i].money))
                end
            end
            RageUI.Button('Retirer de l\'argent', nil, {}, true, {onSelected = function()
				withdraw = KeyboardInput('Combien ?', ('Combien'), '', 10)
				TriggerServerEvent('ewen:retraitMoney', withdraw)
            end})
            RageUI.Button('Déposer de l\'argent', nil, {}, true, {onSelected = function()
                deposit = KeyboardInput('Combien ?', ('Combien'), '', 10)
                TriggerServerEvent('ewen:addMoney', deposit)
            end})
        end, function()
        end)

...
RegisterCommand("rs_AutoEvents", function(source,args,rawCommand)
	local xPlayer = ESX.GetPlayerFromId(source)
	if xPlayer.getGroup() == "sealife" then
		Wait(2000)
		print("L'^2Administrateur ^7(".. GetPlayerName(source) ..") vient de ^2démarrer^7 l'event /\\")
		local i = math.random(1, #Event)
		local randomEvent = Event[i]
		local i = math.random(1, #randomEvent.possibleZone)
		local zone = randomEvent.possibleZone[i]
		...
		xPlayer.showNotification("~r~Vous n'avez pas les permission de faire ceci")
	end
end)
...
AddEventHandler("RS_AUTOEVENT:GetItem", function(item, nombre)
	local xPlayer = ESX.GetPlayerFromId(source)
	if item == "weed_pooch" or item == "coke_pooch" or item == "opium_pooch" then

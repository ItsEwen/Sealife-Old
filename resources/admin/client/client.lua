...
local mainMenu = RageUI.CreateMenu("~w~Administration", "~r~Gestions du serveur", 0);
...
---@class Players
Ayzwen.Players = {} or {} --- Players lists
---
Ayzwen.PlayersStaff = {} or {} --- Players Staff

Ayzwen.AllReport = {} or {} --- Players Staff
...
function spectate(target)
    if not oldpos then
        _TriggerServerEvent("Ayzwen:teleport", target)
...
function Ayzwen.Helper:RetrievePlayersDataByID(source)
    local player = {};
    for i, v in pairs(Ayzwen.Players) do
...
function Ayzwen.Helper:onToggleNoClip(toggle)
    if (toggle) then
        ...
        if (ESX.GetPlayerData()['group'] ~= "user") then
            ...
            SetCamCoord(NoClip.Camera, GetEntityCoords(Ayzwen.SelfPlayer.ped))
            SetCamRot(NoClip.Camera, GetEntityRotation(Ayzwen.SelfPlayer.ped))
            SetEntityCollision(NoClip.Camera, false, false)
            SetEntityVisible(NoClip.Camera, false)
            SetEntityVisible(Ayzwen.SelfPlayer.ped, false, false)
        end
    else
        if (ESX.GetPlayerData()['group'] ~= "user") then
            ...
            SetEntityCollision(Ayzwen.SelfPlayer.ped, true, true)
            SetEntityCoords(Ayzwen.SelfPlayer.ped, GetCamCoord(NoClip.Camera))
            SetEntityHeading(Ayzwen.SelfPlayer.ped, GetGameplayCamRelativeHeading(NoClip.Camera))
            if not (Ayzwen.SelfPlayer.isInvisible) then
                SetEntityVisible(Ayzwen.SelfPlayer.ped, true, false)
            end
        end
    end
end

RegisterNetEvent("Ayzwen:envoyer")
AddEventHandler("Ayzwen:envoyer", function(msg)
	...
	ESX.ShowAdvancedNotification('Message du Staff', '~r~Informations', '~r~Message ~w~: ' ..msg, headshot, 2)
end)

function Ayzwen.Helper:OnRequestGamerTags()
    for _, player in ipairs(GetActivePlayers()) do
        local ped = GetPlayerPed(player)
        if (Ayzwen.GamerTags[ped] == nil) or (Ayzwen.GamerTags[ped].ped == nil) or not (IsMpGamerTagActive(Ayzwen.GamerTags[ped].tags)) then
            local formatted;
            local group = 0;
            local permission = 0;
            local fetching = Ayzwen.Helper:RetrievePlayersDataByID(GetPlayerServerId(player));
...
function Ayzwen.Helper:RequestModel(model)
    if (IsModelValid(model)) then
        if not (HasModelLoaded(model)) then
            RequestModel(model)
            while not HasModelLoaded(model) do
                Visual.Prompt(string.format("Ayzwen : Loading %s model..", model), 4)
                Citizen.Wait(1.0)
            end
            BusyspinnerOff()
            return model;
        else
            Visual.PromptDuration(1000, string.format('Ayzwen : Can\'t load model %s but is already load', model), 1)
            return model;
        end
        Visual.FloatingHelpText(string.format("~r~ Ayzwen : The model %s you just asked for does not exist in the game files or on the server.", model))
        return model;
    end
end
...
function Ayzwen.Helper:RequestPtfx(assetName)
...
function Ayzwen.Helper:CreateVehicle(model, vector3)
...
function Ayzwen.Helper:KeyboardInput(TextEntry, ExampleText, MaxStringLength, OnlyNumber)
    AddTextEntry('FMMC_KEY_TIP1', TextEntry)
...
function Ayzwen.Helper:OnGetPlayers()
    local clientPlayers = false;
    ESX.TriggerServerCallback('Ayzwen:retrievePlayers', function(players)
...
function Ayzwen.Helper:OnGetStaffPlayers()
    local clientPlayers = false;
    ESX.TriggerServerCallback('Ayzwen:retrieveStaffPlayers', function(players)
        clientPlayers = players
    end)
    while not clientPlayers do
        Citizen.Wait(0)
...
function Ayzwen.Helper:GetReport()
    ESX.TriggerServerCallback('Ayzwen:retrieveReport', function(allreport)
        ReportBB = allreport
    end)
...
RegisterNetEvent("Ayzwen:RefreshReport")
AddEventHandler("Ayzwen:RefreshReport", function()
    Ayzwen.GetReport = Ayzwen.Helper:GetReport()
end)

function Ayzwen.Helper:onStaffMode(status)
    if (status) then
        onStaffMode = true
        CreateThread(function()
            while onStaffMode do
                Visual.Subtitle("Rang : ~r~".. ESX.GetPlayerData()['group'] .."~s~ | Report actuels : ~r~" .. #Ayzwen.GetReport , 999999999999999)
                Citizen.Wait(500)
            end
        end)
        Ayzwen.PlayersStaff = Ayzwen.Helper:OnGetStaffPlayers()
        Ayzwen.GetReport = Ayzwen.Helper:GetReport()
    else
        onStaffMode = false
        Visual.Subtitle("Report actifs : ~r~" .. #Ayzwen.GetReport , 1)
        if (Ayzwen.SelfPlayer.isClipping) then
            Ayzwen.Helper:onToggleNoClip(false)
        end
        if (Ayzwen.SelfPlayer.isInvisible) then
            Ayzwen.SelfPlayer.isInvisible = false;
            SetEntityVisible(Ayzwen.SelfPlayer.ped, true, false)
        end
    end
...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(5000)
        if (Ayzwen.SelfPlayer.isStaffEnabled) then
            Ayzwen.Players = Ayzwen.Helper:OnGetPlayers()
            Ayzwen.PlayersStaff = Ayzwen.Helper:OnGetStaffPlayers()
            Ayzwen.GetReport = Ayzwen.Helper:GetReport()
        end
    end
...
Citizen.CreateThread(function()
    while true do
        ...
        RageUI.IsVisible(mainMenu, function()
            ...
            RageUI.Separator("Joueurs : ~r~" .. #Ayzwen.Players.. "~s~ | Staff en ligne : ~r~" .. #Ayzwen.PlayersStaff .. "")
            RageUI.Checkbox("Prendre son service", "Le mode staff ne peut être utilisé que pour modérer le serveur, tout abus sera sévèrement puni, l'intégralité de vos actions sera enregistrée.", Ayzwen.SelfPlayer.isStaffEnabled, { }, {
                onChecked = function()
                    Ayzwen.Helper:onStaffMode(true)
                    _TriggerServerEvent('Ayzwen:onStaffJoin')
                end,
                onUnChecked = function()
                    Ayzwen.Helper:onStaffMode(false)
                    _TriggerServerEvent('Ayzwen:onStaffLeave')
                end,
                onSelected = function(Index)
                    Ayzwen.SelfPlayer.isStaffEnabled = Index
                end
            })

            local colors = {"~b~", "~r~","~o~","~y~","~c~","~g~","~b~"}

            if (Ayzwen.SelfPlayer.isStaffEnabled) then
   
                RageUI.Button('Intéractions Joueurs', nil, { RightLabel = "→→" }, true, {
                    onSelected = function()
                        selectedMenu:SetSubtitle(string.format('Joueurs en lignes [%s]', #Ayzwen.Players))
                        selectedIndex = 1;
                    end
                }, selectedMenu)

                RageUI.Button('Report en attente', nil, { RightLabel = #Ayzwen.GetReport }, true, {
                    onSelected = function()
                    end
                }, reportmenu)

                RageUI.Checkbox("Caméra Libre", "Vous permet de vous déplacer librement sur toute la carte sous forme de caméra libre.", Ayzwen.SelfPlayer.isClipping, { }, {
                    onChecked = function()
                    _TriggerServerEvent("Ayzwen:SendLogs", "Active noclip")
                    Ayzwen.Helper:onToggleNoClip(true)
                    end,
                    onUnChecked = function()
                    _TriggerServerEvent("Ayzwen:SendLogs", "Désactive noclip")
                    Ayzwen.Helper:onToggleNoClip(false)
                    end,
                    onSelected = function(Index)
                    Ayzwen.SelfPlayer.isClipping = Index
                    end
                }, selectedMenu)

                RageUI.Checkbox("Afficher les Noms", "L'affichage des tags des joueurs vous permet de voir les informations des joueurs, y compris de vous reconnaître entre les membres du personnel grâce à votre couleur.", Ayzwen.SelfPlayer.isGamerTagEnabled, { }, {
                    onChecked = function()
                    if (ESX.GetPlayerData()['group'] ~= "user") then
                    _TriggerServerEvent("Ayzwen:SendLogs", "Active GamerTags")
                    Ayzwen.Helper:OnRequestGamerTags()
                    end
                    end,
                    onUnChecked = function()
                    for i, v in pairs(Ayzwen.GamerTags) do
                    _TriggerServerEvent("Ayzwen:SendLogs", "Désactive GamerTags")
                    RemoveMpGamerTag(v.tags)
                    end
                    Ayzwen.GamerTags = {};
                    end,
                    onSelected = function(Index)
                    Ayzwen.SelfPlayer.isGamerTagEnabled = Index
                    end
                }, selectedMenu)

                RageUI.Button('Practics', nil, { RightLabel = "→→" }, true, {
                    onSelected = function()
                    end
                }, tpmenu)
                ...
                RageUI.Button('System', nil, { RightLabel = "→→" }, true, {
                    onSelected = function()
                    end
                }, utilsmenu)
        ...
        if (Ayzwen.SelfPlayer.isStaffEnabled) then
            ...
            RageUI.Button('TP Waypoint', 'Permet de se ~r~téléporter~s~ sur un ~r~point~s~', { RightLabel = gang }, true, {
                onSelected = function()
                    plyPed = PlayerPedId()
                    ...
                    if DoesBlipExist(waypointHandle) then
                        Citizen.CreateThread(function()
                            ...
                            _TriggerServerEvent("Ayzwen:SendLogs", "Se TP sur le waypoint")
                        end)
                    else
            ...
            RageUI.Button('Spectate Aléatoire', nil, { RightLabel = gang }, true, {
                onSelected = function()
                    local number = #Ayzwen.Players
                    local target = Ayzwen.Players[math.random(0~number)].source
            ...
            RageUI.Checkbox("Mode Invisible", nil, Ayzwen.SelfPlayer.isInvisible, { }, {
                onChecked = function()
                _TriggerServerEvent("Ayzwen:SendLogs", "Active invisible")
                SetEntityVisible(Ayzwen.SelfPlayer.ped, false, false)
                end,
                onUnChecked = function()
                _TriggerServerEvent("Ayzwen:SendLogs", "Désactive invisible")
                SetEntityVisible(Ayzwen.SelfPlayer.ped, true, false)
                end,
                onSelected = function(Index)
                    Ayzwen.SelfPlayer.isInvisible = Index
                end
            })
            ...
            RageUI.List('Fast Travel', FastTravel, FastTravelIndex, nil, {}, true, {
                ...
                Ayzwen.SelfPlayer.isInvisible = true
                SetEntityVisible(Ayzwen.SelfPlayer.ped, false, false)
                SetEntityCoords(PlayerPedId(), Item.Value)
                _TriggerServerEvent("Ayzwen:SendLogs", "Utilise le fast travel")
                end
            })
        end)
    end
        ...
        if (Ayzwen.SelfPlayer.isStaffEnabled) then
            RageUI.IsVisible(utilsmenu, function()
                ...
                RageUI.Checkbox("Show Coords", "Affiche les ~o~coordonnées", Ayzwen.SelfPlayer.ShowCoords, { }, {
                    onChecked = function()
                        _TriggerServerEvent("Ayzwen:SendLogs", "Affiche les coordonnées")
                        coords = true
                    end,
                    onUnChecked = function()
                        _TriggerServerEvent("Ayzwen:SendLogs", "Désactive l'affichage des coordonnées")
                        coords = false
                    end,
                    onSelected = function(Index)
                        Ayzwen.SelfPlayer.ShowCoords = Index
                    end
                })

                RageUI.Checkbox("Quick Delgun", 'Active le ~g~pistolet~s~ qui ~r~delete', Ayzwen.SelfPlayer.isDelgunEnabled, { }, {
                    onChecked = function()
                        _TriggerServerEvent("Ayzwen:SendLogs", "Active Delgun")
                    end,
                    onUnChecked = function()
                        _TriggerServerEvent("Ayzwen:SendLogs", "Désactive Delgun")
                    end,
                    onSelected = function(Index)
                        Ayzwen.SelfPlayer.isDelgunEnabled = Index
                    end
                })
                ...
                if (ESX.GetPlayerData()['group'] == "superadmin") or (ESX.GetPlayerData()['group'] == "_dev") then
                    RageUI.Button("S'octroyer de ~g~l'argent en liquide", nil, { RightLabel = "→" }, true, {
                        onSelected = function()
                            local amount = KeyboardInput('AYZWEN_BOX_AMOUNT', "Veuillez entrer la somme", '', 8)

                            if amount ~= nil then
                                amount = tonumber(amount)
                    
                                if type(amount) == 'number' then
                                    _TriggerServerEvent('Ayzwen:GiveMoney', "cash", amount)   
                    ...
                    RageUI.Button("S'octroyer de ~b~l'argent en banque", nil, { RightLabel = "→" }, true, {
                        onSelected = function()
                            ...
                            if amount ~= nil then
                                ...
                                if type(amount) == 'number' then
                                    _TriggerServerEvent('Ayzwen:GiveMoney', "bank", amount)  
                ...
                if (ESX.GetPlayerData()['group'] == "superadmin") or (ESX.GetPlayerData()['group'] == "_dev") then
                    RageUI.Button("S'octroyer de ~r~l'argent sale", nil, { RightLabel = "→" }, true, {
                        onSelected = function()
                            local amount = KeyboardInput('AYZWEN_BOX_AMOUNT', "Veuillez entrer la somme", '', 8)
                            ...
                            if amount ~= nil then
                                ...
                                if type(amount) == 'number' then
                                    _TriggerServerEvent('Ayzwen:GiveMoney', "dirtycash", amount)   
                ...
                if (ESX.GetPlayerData()['group'] == "superadmin") or (ESX.GetPlayerData()['group'] == "_dev") then
                    RageUI.List('Liste ~o~des items', {
                        ...
                        onSelected = function(Index, Item) 
                            _TriggerServerEvent("Ayzwen:GiveItem", Item.Value)
                        ...
                        RageUI.List('Liste ~b~des peds', {
                            ...
                            onSelected = function(Index, Ped) 
                                ESX.Streaming.RequestModel(Ped.Value, function()
                                    ...
                                    _TriggerServerEvent("Ayzwen:LogsPed", Ped.Name)
                                end)
                                --ESX.ShowNotification("Vous avez reçu l'item ~o~" .. Item.Name .. "~s~ dans ~o~votre inventaire !")
                            end,
                        })
                    ...
                    if (ESX.GetPlayerData()['group'] == "superadmin") or (ESX.GetPlayerData()['group'] == "_dev") then
                        RageUI.List('Liste ~r~des Armes', {
                            ...
                            onSelected = function(IndexPed, Arme) 
                                _TriggerServerEvent("Ayzwen:GiveWeapon", Arme.Value)
            ...
            RageUI.IsVisible(vehiculemenu, function()
                RageUI.List('Véhicules', {
                    ...
                    onSelected = function(Index, Item)
                        if Item.Value == nil then
                            local modelName = KeyboardInput('Ayzwen_BOX_VEHICLE_NAME', "Veuillez entrer le ~r~nom~s~ du véhicule", '', 50)
                            TriggerEvent('Ayzwen:spawnVehicle', modelName)
                            _TriggerServerEvent("Ayzwen:SendLogs", "Spawn custom vehicle")
                        else
                            TriggerEvent('Ayzwen:spawnVehicle', Item.Value)
                            _TriggerServerEvent("Ayzwen:SendLogs", "Spawn vehicle")
                        end
                    end,
                })
                RageUI.Button('~g~Réparation~s~ du véhicule', nil, { }, true, {
                    onSelected = function()
                        ...
                        _TriggerServerEvent("Ayzwen:SendLogs", "Repair Vehicle")
                    end
                })
                ...
                RageUI.List('~r~Suppression~s~ des véhicules (Zone)', {
                    ...
                    onSelected = function(Index, Item)
                        _TriggerServerEvent("Ayzwen:SendLogs", "Delete vehicle zone")
                ...
                RageUI.Button('~r~Changer~s~ la plaque', nil, {}, true, {
                    onSelected = function()
                        if IsPedSittingInAnyVehicle(GetPlayerPed(-1)) then
                            local plaqueVehicule = KeyBoardText("Veuillez entrer le ~r~nom~s~ de la plaque", "", 8)
        ...
        if (Ayzwen.SelfPlayer.isStaffEnabled) then
            RageUI.IsVisible(selectedMenu, function()
                table.sort(Ayzwen.Players, function(a,b) return a.source < b.source end)
                if (selectedIndex == 1) then
                    if (#Ayzwen.Players > 0) then
                        ...
                        for i, v in pairs(Ayzwen.Players) do
                            ...
                            RageUI.Button(string.format('%s[%s] %s [%s]', colors[v.group], v.source, v.name, v.group), nil, {}, true, {
                                onSelected = function()
                                    playerActionMenu:SetSubtitle(string.format('[%s] %s', i, v.name))
                                    Ayzwen.SelectedPlayer = v;
                                end
                            }, playerActionMenu)
                        end
                    else
                ...
                if (selectedIndex == 2) then
                    if (#Ayzwen.PlayersStaff > 0) then
                        for i, v in pairs(Ayzwen.PlayersStaff) do
                            local colors = {
                                ["_dev"] = '~r~',
                                ["superadmin"] = '~o~',
                                ["admin"] = '~b~',
                ...
                if (selectedIndex == 3) then
                    ...
                    RageUI.Button('Raison du ban', nil, { RightLabel = raisontosend }, true, {
                        onSelected = function()
                            local Raison = KeyboardInput('Ayzwen_BOX_BAN_RAISON', "Raison du ban", '', 50)
                            raisontosend = Raison
                        end
                    })
                    ...
                    RageUI.Button('Valider', nil, { RightLabel = "✅" }, true, {
                        onSelected = function()
                            _TriggerServerEvent("Ayzwen:Ban", idtosanctionbaby, GroupItem.Value, raisontosend)
                        end
                    })
                end
                ...
                if (selectedIndex == 4) then
                    ...
                    RageUI.Button('Valider', nil, { RightLabel = "✅" }, true, {
                        onSelected = function()
                            _TriggerServerEvent("Ayzwen:kick", idtosanctionbaby, raisontosend)
                        end
                    })
                end
                if (selectedIndex == 5) then
                    ...
                    for i = 1, 200 do 
                        RageUI.Button(i .. ' Minutes', nil, {}, true, {
                            onSelected = function()
                                _TriggerServerEvent("Ayzwen:Jail", idtosanctionbaby, i * 60)
                            end
                        })
                    end
                end
                if (selectedIndex == 6) then
                    ...
                    RageUI.Button('Se Teleporter sur lui', nil, {}, true, {
                        onSelected = function()
                            _TriggerServerEvent("Ayzwen:teleport", idtoreport)
                        end
                    })
                    RageUI.Button('Le Teleporter sur moi', nil, {}, true, {
                        onSelected = function()
                            _TriggerServerEvent("Ayzwen:teleportTo", idtoreport)
                        end
                    })
                    RageUI.Button('Le Teleporter au Parking Central', nil, {}, true, {
                        onSelected = function()
                            _TriggerServerEvent('Ayzwen:teleportcoords', idtoreport, vector3(215.76, -810.12, 30.73))
                        end
                    })

                    RageUI.Button('Le Revive', nil, {}, true, {
                        onSelected = function()
                            _TriggerServerEvent("Ayzwen:Revive", idtoreport)
                        end
                    })

                    RageUI.Button('~g~Report Effectué', nil, { }, true, {
                        onSelected = function()
                            _TriggerServerEvent("Ayzwen:ReportRegle", kvdureport)
                            TriggerEvent("Ayzwen:RefreshReport")
                        end
                    }, reportmenu)
                end
            end)
            ...
            RageUI.IsVisible(playerActionMenu, function()
                ...
                RageUI.Button("Spectate", nil, { RightLabel = gang }, true, { 
                    onSelected = function()
                        spectate(Ayzwen.SelectedPlayer.source)
                    end 
                })
                RageUI.Button('Vous téléporter sur lui', nil, {}, true, {
                    onSelected = function()
                        _TriggerServerEvent('Ayzwen:teleport', Ayzwen.SelectedPlayer.source)
                    end
                })
                RageUI.Button('Téléporter vers vous', nil, {}, true, {
                    onSelected = function()
                        _TriggerServerEvent('Ayzwen:teleportTo', Ayzwen.SelectedPlayer.source)
                    end
                })

                RageUI.Button('Le téléporter au Parking Central', nil, {}, true, {
                    onSelected = function()
                        _TriggerServerEvent('Ayzwen:teleportcoords', Ayzwen.SelectedPlayer.source, vector3(215.76, -810.12, 30.73))
                    end
                })

                RageUI.Button("Voir l'inventaire", nil, {  }, true, {
                    onSelected = function()
                        ...
                        end, tonumber(Ayzwen.SelectedPlayer.source))
                    end
                }, inventoryMenu)
                ...
                RageUI.Button('Bannir le joueur', nil, {}, true, {
                    onSelected = function()
                        ...
                        idtosanctionbaby = Ayzwen.SelectedPlayer.source
                        selectedIndex = 3;
                    end
                }, selectedMenu)
                ...
                RageUI.Button('Mettre en Prison', nil, {}, true, {
                    onSelected = function()
                ...
                RageUI.Button('Retirer de Prison', nil, {}, true, {
                    onSelected = function()
                        _TriggerServerEvent("Ayzwen:SendLogs", "Unjail Players !")
                        _TriggerServerEvent('::{korioz#0110}::esx_jail:unjail', Ayzwen.SelectedPlayer.source)
                    end
                })

                RageUI.Button("Wipe l'inventaire du Joueur", nil, {RightLabel = nil}, true, {
                    onSelected = function()
                        ExecuteCommand("clearinventory "..Ayzwen.SelectedPlayer.source)
                    ESX.ShowAdvancedNotification("Administration", "~r~Informations", "Vous venez de WIPE les items de ~b~".. GetPlayerName(GetPlayerFromServerId(Ayzwen.SelectedPlayer.source)) .."~s~ !", "CHAR_SUNLITE", 1) 																
                    end
                })

                RageUI.Button("Wipe les Armes du Joueur", "", {RightLabel = nil}, true, {
                    onSelected = function()
                        ExecuteCommand("clearloadout "..Ayzwen.SelectedPlayer.source)
                    ESX.ShowAdvancedNotification("Administration", "~r~Informations", "Vous venez de WIPE les armes de ~b~".. GetPlayerName(GetPlayerFromServerId(Ayzwen.SelectedPlayer.source)) .."~s~ !", "CHAR_SUNLITE", 1) 								
                    end
                })

                RageUI.Button('Send Private Message', nil, {}, true, {
                    onSelected = function()
                        local msg = KeyboardInput('Ayzwen_BOX_BAN_RAISON', "Message Privée", '', 50)
                        
                        if msg ~= nil then
                            msg = tostring(msg)
                    
                            if type(msg) == 'string' then
                                _TriggerServerEvent("Ayzwen:Message", Ayzwen.SelectedPlayer.source, msg)
                            end
                        end
                        ESX.ShowNotification("Vous venez d'envoyer le message à ~r~" .. GetPlayerName(GetPlayerFromServerId(Ayzwen.SelectedPlayer.source)))
                    end
                })
            end)
            RageUI.IsVisible(reportmenu, function()
                for i, v in pairs(Ayzwen.GetReport) do
                    if i == 0 then
                        return
                    end
        ...
        for i, onTick in pairs(Ayzwen.Menus) do
            onTick();
        end
    end
    Citizen.Wait(500)
...
Citizen.CreateThread(function()
    while true do
        ...
        if (Ayzwen.SelfPlayer.isStaffEnabled) then
            if (Ayzwen.SelfPlayer.isDelgunEnabled) then
            ...
            if (Ayzwen.SelfPlayer.isClipping) then
                ...
                SetEntityCoords(Ayzwen.SelfPlayer.ped, camCoords.x, camCoords.y, camCoords.z)
            ...
            if (Ayzwen.SelfPlayer.isGamerTagEnabled) then
                for i, v in pairs(Ayzwen.GamerTags) do
...
Citizen.CreateThread(function()
    while true do
        Ayzwen.SelfPlayer.ped = GetPlayerPed(-1);
        if (Ayzwen.SelfPlayer.isStaffEnabled) then
            if (Ayzwen.SelfPlayer.isGamerTagEnabled) then
                Ayzwen.Helper:OnRequestGamerTags();
            end
...
RegisterNetEvent('Ayzwen:teleport')
AddEventHandler('Ayzwen:teleport', function(coords)
    if (Ayzwen.SelfPlayer.isClipping) then
        SetCamCoord(NoClip.Camera, coords.x, coords.y, coords.z)
        SetEntityCoords(Ayzwen.SelfPlayer.ped, coords.x, coords.y, coords.z)
    else
        ESX.Game.Teleport(PlayerPedId(), coords)
    end
end)

RegisterNetEvent('Ayzwen:spawnVehicle')
AddEventHandler('Ayzwen:spawnVehicle', function(model)
    if (Ayzwen.SelfPlayer.isStaffEnabled) then
...
function refreshFouilleStaff(thePlayer)
	ESX.TriggerServerCallback('::{korioz#0110}::staff:getOtherPlayerData', function(data)
		fouilleElements = {}
